...
import com.github.yuzhian.zero.boot.support.ApiResponse;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.httpBasic()
                ...
                .logoutSuccessHandler((request, response, authentication) ->
                        responseObject(response, ApiResponse.of(0, "已登出")))
    ...
    private UsernamePasswordAuthenticationFilter authenticationFilter() throws Exception {
        ...
        authenticationFilter.setAuthenticationSuccessHandler((request, response, authentication) -> {
            // 认证成功返回 sessionId
            responseObject(response, Map.of(GlobalConfigConstants.AUTHORIZATION, request.getSession().getId()));
        });
