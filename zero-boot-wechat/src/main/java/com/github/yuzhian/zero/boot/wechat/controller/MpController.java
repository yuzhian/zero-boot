...
public class MpController extends BaseController {
    ...
    @Operation(summary = "微信消息验证", description = "供微信验证服务器资源", hidden = true)
    ...
    @Operation(summary = "微信消息处理", description = "微信消息处理", hidden = true)
    public String handleMessage(String timestamp, String nonce, String signature,
        ...
        if (log.isDebugEnabled())
            log.debug("message encrypt type: [{}], content：[{}]", encryptType, request.toString());
